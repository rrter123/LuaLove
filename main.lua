...
function love.keypressed(key, scancode, isrepeat)
  ...
  --[[
  if (key == "right" or key == "d") and check.can_go_right(player.player_x, player.player_y, current_map)
  then
    offset_x =offset_x + 100
    player.player_x = player.player_x+1
  end
  if (key == "left" or key == "a") and check.can_go_left(player.player_x, player.player_y, current_map)
  then
    offset_x =offset_x - 100
    player.player_x = player.player_x-1
  end
  if (key == "down" or key == "s") and check.can_go_down(player.player_x, player.player_y, current_map)
  then
    offset_y =offset_y + 100
    player.player_y = player.player_y+1
  end
  if (key == "up" or key == "w") and check.can_go_up(player.player_x, player.player_y, current_map)
  then
    offset_y =offset_y - 100
    player.player_y = player.player_y-1
  end
